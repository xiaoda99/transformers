...
def predict(model, tokenizer, text, examples, k_shot=3, bos_token=' ->', eos_token=None, #'Ċ',
    ...
    else: cxt, query, cands, *_ = examples[0]
    if cands is not None and (cands[-2] == cands[-1] or is_cls_candidates(cands[-1])) and logits_mask is not None: # rel1 is copy or g2c task
        print('In predict: set logits_mask') # wab check
    ...
    # o.labels_mask = labels_mask  # wab remove
...
def generate_and_predict_batch(model, tokenizer, task, nrows, k_shot, batch_size,
            logits_bias=None, ioi_dataset=None, wino_dataset=None ,by_head=None, trim=False,custom_forward=True,
    ...
    if result is None:
        ...
        elif wino_dataset is not None:  # wab
            indices_groups = np.arange(batch_size * nrows).reshape(batch_size, nrows)
            texts, all_examples, all_bos_tokens = [], [], []
            for indices in indices_groups:
                # texts.append('\n'.join(ioi_dataset.sentences[i] for i in indices) + '\n')#wab
                texts.append(''.join(wino_dataset.sentences[i] for i in indices))#wab
                all_examples.append([{k: wino_dataset.word_idx[k][i] for k in wino_dataset.word_idx} for i in indices])
                all_bos_tokens.append([wino_dataset.word_idx['end'][i] for i in indices])
        else:
...
def sum_forward(model, outputs, labels=None, loss_reduction='per_example_mean',
    ...
    # for lm_head_forward
    kwargs = dict(labels=labels, loss_reduction=loss_reduction, scaled=scaled)
...
wino_attn_patterns_by_step = {-1: ['bos->ans0','bos->io','bos->s1'],
    ...
    1: [ 's2->s1','s2->s1+','s1->perp','s1->v','bos->io','s1->io','bos->v','s1->ans0','io->ans0','bos->s1','bos->perp','bos->s2','nans0->ans0','bos->nans0+'],
    2: [ 'inter->ansright','s2->s1','s2->s1+','s1+->s1', 's1->perp','s1->v','bos->io','s1->io','s1->ans0','nans0+->nans0','bos->s2','bos->s2+','bos->nans0+','bos->inter','bos->ansright'], 
    3: ['ansright->nans0','inter->s2','s1->perp','s1->v','s2->v','s2->perp','bos->io','s2->io','s1->io','perp->io','perp->s2','perp->v','s1->ans0','perp->ans0','s2->ans0','io->s2','bos->s1','bos->ansright','bos->perp'],
    4:['s1->perp','s1->v','s2->ansright','bos->io','s1->io','io->perp','io->v','io->s2','s1->ans0','perp->io','perp->s2','perp->v','perp->ans0','s2->ans0','bos->s1','inter->io','inter->perp','inter->v','bos->ansright','bos->perp','bos->inter'],
    5:['v->s2','v->ans0','io->v','io->s2','s1->ans0','perp->io','perp->s2','perp->v','perp->ans0','s2->ans0','s1->v','s1->io','s1->perp','s1->inter','inter->op','bos->perp'],
    6:['bos->inter','bis->s1','s2->ansright',''],
    }

wino_attn_patterns_by_step_v2 = {-1: ['bos->ans0','bos->nans0'],
    0: [ 'bos->s1', 'bos->ans0+', 'bos->s2', 'bos->inter','bos->relation','bos->io','bos->nans0','s1->relation','bos->relation'],
    ...
    2: [ 's1->ansright','ansright->postans','bos->io','inter->ansright','s1->inter','s1+->s1','bos->io','s1->io','s1->ans0','nans0+->nans0','bos->s2','bos->s2+','bos->nans0+','bos->inter','bos->ansright','s1->relation','inter->relation','relation->s2'], 
    3: ['s1->nans0+','ansright->nans0','inter->s2','bos->io','s2->io','s1->io','s1->ans0','s2->ans0','relation->ansright','bos->s1','bos->ansright','s1->inter','relation->ans0','relation->nans0','relation->io','relation->s2'],
...
all_attn_patterns = join_lists(wino_attn_patterns_by_step_v2.values())# + ['bos->bos']#wab
...
def get_label_types_bk(parent, d, k_shot=3, icl_score_thld=0.3): # use fixed k_shot #mqy default icl score: 0.4
...
def get_label_types(parent, d, k_shot=3, icl_score_thld=0.3): # use fixed k_shot #mqy default icl score: 0.4
    ...
    if is_root(parent) and point_wise(d.attn_pattern):
        label_types = ['labels']  # e.g. bos->bos at step 0
    elif icl_score > icl_score_thld or is_predicting_head or is_recurrent_head or d.attn_pattern in ['bos->ansright']: #or d.ap_score >0.99: #mqy#wab
        label_types = ['attn_labels', normalized_attn_labels][:1] if not point_wise(d.attn_pattern) else [None]
        activating_attn_labels = f'attn_labels:{parse_attn_pattern(d.attn_pattern)[0]}->~<s>,{min(3, k_shot)}'
        # head_label_types = get_root(parent).data.head_label_types
        # head_label_type = (d.layer, d.head, activating_attn_labels)
    ...
    if plot_attr:
        ...
        *_, ys, logits = attribute(fwd_fn, model, x, [fn] + fns, num_points=3+4, forward_only=True)
        ...
        if logits is None: pass
            ...
            # logits = logits + o.logits_mask #* (1e4 if logits.dtype == torch.float16 else 1e9)
            ap_scores, corrects, *_ = show_predictions(tokenizer, *args, logits=logits[-1:], labels=labels,
