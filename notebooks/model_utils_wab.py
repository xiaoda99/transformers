...
def scaled_ln(ln, x, mean=None, std=None, scaled=True):
    ...
    if std is None:
        ...
        if scaled: std = std[-1:].detach()  # use the std of the lasbosuit original (unscaled) example in the batch
...
def predict(model, tokenizer, text, examples, k_shot=3, bos_token=' ->', eos_token=None, #'Ċ',
    ...
    if False and cands is not None and cands[-2] == cands[-1] and logits_mask is not None: # rel1 is copy
...
def generate_and_predict_batch(model, tokenizer, task, nrows, k_shot, batch_size,
    ...
    # if batch_size == 1: return result#wab
...
wino_attn_patterns_by_step = {-1: ['bos->ans0'],
    0: [ 'bos->s1', 'bos->ans0+', 'bos->s2', 'bos->s2+','bos->rel','bos->v','bos->perp','bos->io','bos->v'],
    1: [ 's1->s2','s1->s2+','s1->perp','s1->v','bos->io','s1->io','bos->v','s1->ans0'],
    2: [ 's1->s2','s1->s2+','s1+->s1', 's1->perp','s1->v','bos->io','s1->io','s1->ans0'], 
    3: ['s1->perp','s1->v','s2->v','s2->perp','bos->io','s2->io','s1->io','perp->io','perp->s2','perp->v','s1->ans0','perp->ans0','s2->ans0'],
    4:['s1->perp','s1->v','bos->io','s1->io','io->perp','io->v','io->s2','s1->ans0','perp->io','perp->s2','perp->v','perp->ans0','s2->ans0'],
    5:['s1->perp','s1->v','bos->io','s1->io','io->perp','io->v','io->s2','s1->ans0','perp->io','perp->s2','perp->v','perp->ans0','s2->ans0']}


all_attn_patterns = join_lists(wino_attn_patterns_by_step.values())# + ['bos->bos']
...
def filter_fn(p, c):  
    ...
    return (p.step == -1 and c.ap_score > 0.15 and c.top_score > 0.4  or # and ap.startswith('B->A0') or  and c.icl_score > 0.05
       ...
       p.step == 1  and (c.label_type == None or not c.label_type.startswith('attn_labels')) or #and ((ap.startswith('B->Q') and c.top_score>0.15) or ap.startswith('B->A]') or ap.startswith('Q->A0'))
...
def attribute_tree(data_tuples, model, node, max_step, filter_fn, topk=10, threshold_score=0.3,
    ...
    # device = device or model.lm_head.weight.device#wab
    device = device
...
def plot_attn_attr(data_tuple, model, tokenizer, node, l, h, attn_patterns=None,
    ...
    if True: #with Timer():
        ...
        for ax, a in zip(axs, attns):
            fontsize = 30 if ystop - ystart == aw_size[0] else 10
