...
def mixed_forward(model, hidden_states, layer, head, labels=None, label_type=None, attn_pattern=None,
    ...
    logits_after_sum = label_type and 'attn_labels' not in label_type # TODO: should always be True?
...
def predict_wab(model,tokenizer,dataset,custom_forward=True, trim=False, num_sp = 10, result=None):
    ...
    for i,inp in enumerate(dataset.sentences):
        count +=1
...
def data2str(data, verbose=True):
    ...
    if not isinstance(layer, Iterable):
        ...
        def suffix(score): return f' {int(round(score*100))}' if verbose and \
            top_score is not None and (lt if step < 2 else ge)(score, 0.4) else ''
        ...
        if verbose and attn_pattern is not None:
            s += f' {abbreviate_attn_pattern(attn_pattern)} {int(round(attr_ap_score * 100))}'#wab
...
def expand_node(parent, head_attr_fn=None, topk=10, threshold_score=0.4,
                k_shot=None, attribute_k=False, add_dummy_node=False, verbose=True):
...
# def filter_fn(p, c):
#     return p.step == -1 and c.ap_score > 0.7 and c.top_score > 0 and c.head < c.H or \
#         p.step == 0 and c.attn_pattern in ['bos->query', 'bos->bos'] and c.head < c.H or \
...
def plot_attn_attr(data_tuple, model, tokenizer, node, l, h, attn_patterns=None,
    ...
    if plot_attr:
        ...
        *_, ys, logits = attribute(fwd_fn, model, x, [fn] + fns, num_points=3, forward_only=True); print(ys)
        ...
        if True or isinstance(h, Iterable) or h == H: return
