...
def average_activations(data_tuples, layer, range_label, neuron_idx=None):
    ...
    avg_mlp_gate = torch.stack([o.mlp_gates[layer][0, slice(*get_range(ranges[0], range_label))].mean(0)#wab
    ...
    for text, input_ids, labels, ranges, *_, o in data_tuples:
        ...
        sli = slice(*get_range(ranges[0], range_label))#wab
        if neuron_idx:
            o.avg_mlp_gates[layer][0, sli] = 0
            o.avg_mlp_gates[layer][0, sli, neuron_idx] = avg_mlp_gate[neuron_idx]
            # o.avg_mlp_gates[layer][0, sli, neuron_idx] = 0
            # o.avg_mlp_gates[layer][0, sli, neuron_idx] = 100*o.avg_mlp_gates[layer][0, sli, neuron_idx]#lianxu scale -2 :2
        else:
...
def get_label_types(parent, d, k_shot=3, icl_score_thld=0.3): # use fixed k_shot #mqy default icl score: 0.4
    ...
    if d.step in [1,2,3] and d.attn_pattern in ['bos->s1']:#wab
        label_types = [None,'attn_labels:attribute_k']
    return label_types#wab
