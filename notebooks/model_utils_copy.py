...
class AttrData:
    ...
    icl_ap_score: float = None  # for self
...
def head_forward(model, hidden_states, layer, head, logits_mask=None, labels=None, loss_reduction=None,
    ...
    if labels is not None:
        logits, loss = lm_head_forward(model, head_output, logits_mask=logits_mask, 
...
def show_predictions(tokenizer, example_strs, bos_indices, eos_indices, answers, candidates, answer_indices,
        logits=None, labels=None, mask_logits=False,
    ...
    ans_nlls, logits_mask, labels_mask = (None, torch.zeros_like(logits), torch.zeros_like(labels)) \
        if not use_openai_api and candidates is not None else ([], None, None)
    ...
    for i, (example_str, bos_i, eos_i, ans_ids) in enumerate(zip(example_strs, bos_indices, eos_indices, answers)):
        ...
        for j, ans_id, ans_token in zip(range(len(ans_ids)), ans_ids, ans_tokens):#, ans_probs, ans_prob_dist):
            logits_str = ''
            if use_openai_api:
                ...
                cand_probs_ = [dist.get(cand, 0.) for cand in [t.replace('Ġ', ' ')
                ...
                if j == 0 and candidates is not None:
                    ...
                    # mask_logits=True may produce more top1_corrects than mask_logits=False
                    # where the losses are actually used for attribution
                    labels_mask[:, bos_i + j] = (masked_logits_.argmax() == ans_id).item()
                    logits_str = ' '.join(('*' if cand == ans_id else '') +
                        f'{tokenizer.convert_ids_to_tokens(cand)}:{logits[0, bos_i, cand].item():.3f}'
                        for cand in candidates[i])
                else:
                    labels_mask[:, bos_i + j] = (logits_.argmax() == ans_id).item()
                dist = logits_.softmax(-1)
                ans_prob = round(dist[ans_id].item(), 3)
                top1_correct = (dist.argmax() == ans_id).item()
                cand_probs_ = [dist[cand].item() for cand in candidates[i]]
...
def predict(model, tokenizer, text, examples, k_shot=3, bos_token=' ->', eos_token=None, #'Ċ',
    ...
    labels[labels_mask == 0] == -100  # once evaluation completes, revise labels for attribution
...
def generate_and_predict_batch(model, tokenizer, task, nrows, k_shot, batch_size, ioi_dataset=None,
    ...
    if result is None:
        if ioi_dataset is not None:
            ...
            all_examples, texts, all_bos_tokens = zip(*[generate(task, verbose=False, plot=False, nrows=nrows, tokenizer = tokenizer, **gen_args)
...
def attn_pattern2labels(ranges, attn_pattern, attn_size, k_shot=0, attribute_k=False, normalize=False):
    ...
    ranges_q = ranges[k_shot:] if attribute_k or not is_relating(attn_pattern) else ranges[: -k_shot]
    ...
    for r in ranges_q:
        if is_intermediary(attn_pattern):
    ...
    if any(s in k for s in ['ans0s', 'candidates', '<s>', 'example']): normalize = False  # used as attn_mask
...
def get_sensitivity(data_tuple, l, h, attn_pattern, node_key, **kwargs):
    ...
    if q != 'bos' and not q.endswith(']'): q = q + ']'
    k = '*' if 'bos' in q and 'ans' in k and 'ans0' not in k else 'ans0'#wab label
    attn_labels, _ = attn_pattern2labels(ranges, f'{q}->{k}', aw.size(), **kwargs)
    assert (attn_labels.sum(1) != 0).sum().item() == len(ranges) - kwargs['k_shot'], \
        f"{attn_pattern}, {(attn_labels.sum(1) != 0).sum().item()} != {len(ranges) - kwargs['k_shot']}"  # q is single-token
    b = attn_labels != 0
    _aa = aa * (aa > 0)
        ...
        if verbose:
            ...
            if head < H:
                ...
                if not is_intermediary(attn_pattern):
                    if d.icl_ap_score is not None: s += f'/{round(d.icl_ap_score * 100)}'
...
def get_possible_attn_patterns(parent, ranges):
    ...
    # d = {'ans->ans0': 'ans]->ans0]', 'ans->ans0+': 'ans]->ans0+'}
...
def get_label_types(parent, d, k_shot=3, icl_ap_score_thld=0.5): # use fixed k_shot
    activating_attn_labels = f'attn_labels:bos->~<s>,{k_shot}'
    normalized_attn_labels = 'attn_labels' + ('/ans0s' if d.attn_pattern.startswith('bos->ans0') else '/example')#wab-xd  '/ans0s'->'example'
    head_label_types = get_root(parent).data.head_label_types
    head_label_type = (d.layer, d.head, activating_attn_labels)
    icl_ap_score = d.icl_ap_score or 0
    is_predicting_head = all(point_wise(n.data.attn_pattern) and n.data.label_type is None for n in node2path(parent)) \
        and not point_wise(d.attn_pattern)
    label_types = [normalized_attn_labels if is_predicting_head else None]
    if icl_ap_score > icl_ap_score_thld:
        if head_label_type not in head_label_types:
            label_types += [activating_attn_labels]
            head_label_types.add(head_label_type)
        if not is_predicting_head and not point_wise(d.attn_pattern):
            label_types += [normalized_attn_labels]
    return label_types

def get_icl_ap_score(ap_scores, k_shot=3): # use fixed k_shot#wab k_shot
...
def expand_node(data_tuples, parent, head_attr_fn=None, topk=10, threshold_score=0.3, k_shot=None,
    ...
    assert scores[0] > 0, str(scores[0])  # wab
    ...
    for i, (layer, head, score) in enumerate(zip(layers, heads, scores)):
        ...
        if head < H:
            ...
            if k_shot > 0: # wab + xd: not ioi task
                d.icl_ap_score = 0 if is_intermediary(d.attn_pattern) or is_relating(d.attn_pattern) else \
                    get_icl_ap_score(mr(get_head_matching_scores)(data_tuples, d.attn_pattern,
                    layer=layer, head=head, averaged=False, k_shot=0))#,k_shot = k_shot)  # wab - xd
    ...
    if True: #pd.step < 1 or pd.step == 1 and pd.attn_pattern in ['bos->tgt', 'bos->query']:
        for d in top_data:
            ...
            label_types = get_label_types(parent, d)
        ...
        if pd.step == -1:
            ...
            for layer, head, score in zip(layers, heads, scores):
                ...
                icl_ap_score = get_icl_ap_score(mr(get_head_matching_scores)(data_tuples, top_attn_pattern,
                    layer=layer, head=head, averaged=False, k_shot=0)) if k_shot > 0 else 0 # wab + xd
                for label_type in label_types + (['attn_labels:bos->~<s>,3'] if icl_ap_score > 0.5 else []):
                    add_node(parent, layer=layer, head=head, head_attr_fn=head_attr_fn, label_type=label_type,
                        ...
                        ap_score=score, icl_ap_score=icl_ap_score, verbose=verbose,
...
def filter_fn(p, c):#wab
    ...
    # return p.step == -1 and ap.startswith('B->A0') and c.ap_score > 0.2 and c.top_score > 0.3 or \
    #     p.step == 0 and pap.startswith('B->A0') and (ap.startswith('B->A0') and c.ap_score > 0.2 or ap == 'B->B' and c.ap_score > 0.3 or ap in ['B->Q', 'B->A]']) or \
    #     p.step == 1 and pap == 'B->Q' and ap == 'Q->T'
    return  (p.step == -1 and ap in ['B->A0']) or (p.step == 0 and ap in ['B->s2'] ) or (p.step == 1 and ap in ['s2->s1+','s2->s1'])
...
def plot_attn_attr(data_tuple, model, tokenizer, node, l, h, attn_patterns=None,
    ...
    ystart = bos_indices[k_shot] if k_shot > 0 else 0  # ioi task
    ...
    attns, ystart = ([aw, aa], ystart) if plot_attr else ([aw], 0)
    if attn_patterns and not attribute_k and attn_patterns[0].lower().startswith('a'):
        # 'ans]->ans0]' or 'ans]->ans0+' for relating heads
        ystart, ystop = 0, bos_indices[-(k_shot+2)] + 3
...
all_attn_patterns = join_lists(attn_patterns_by_step.values())# + ['bos->bos']#wab
